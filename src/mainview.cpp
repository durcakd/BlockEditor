    ...
    : QGraphicsView(), _lineNumbersOverlay(NULL), _contextOverlay(NULL)
{
    ...
    connect(verticalScrollBar(), SIGNAL(valueChanged(int)), SLOT(viewScrolled()));
    connect(verticalScrollBar(), SIGNAL(rangeChanged(int, int)), SLOT(viewScrolled()));
}
...
void MainView::viewScrolled()
{
    qDebug() << "DEBUG :: viewScrolled";
    QRectF viewRect = mapToScene(viewport()->rect()).boundingRect();
    emit scopeChanged(viewRect);
}
